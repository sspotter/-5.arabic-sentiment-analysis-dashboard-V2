...
// import { Folder, Trash2, Play, Calendar, FileText, Star } from 'lucide-react';
...
//                       {isDefault && (
//                         <Star className="w-4 h-4 ml-2 text-amber-400 fill-current" title="Default Project" />
//                       )}
//                     </div>
...
export function SavedProjectsList({ projects, onLoad, onDelete, onClose }: SavedProjectsListProps) {
  return (
    <div className="w-full max-w-4xl mx-auto bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-800">
        <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200 flex items-center">
          <Folder className="w-6 h-6 mr-2 text-purple-400" />
          Saved Projects
        </h2>
      ...
      <div className="p-6">
        {projects.length === 0 ? (
          <div className="text-center py-12">
            <Folder className="w-12 h-12 text-slate-700 dark:text-slate-400 mx-auto mb-4" />
            <h3 className="text-lg font-medium text-slate-700 dark:text-slate-300">No saved projects</h3>
            <p className="text-slate-500 dark:text-slate-400 mt-1">Projects you save will appear here.</p>
          </div>
        ) : (
          <div className="grid gap-4">
            {projects.map(p => (
              <div key={p.id} className="flex items-center justify-between p-4 border border-slate-200 dark:border-slate-800 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                <div className="flex-1 min-w-0">
                  <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200 truncate">{p.name}</h3>
                  <div className="flex items-center mt-1 text-sm text-slate-500 dark:text-slate-400 space-x-4">
                    <span className="flex items-center">
                ...
                <div className="flex items-center space-x-2 ml-4">
                  <button 
                    onClick={() => onLoad(p)} 
                    className="inline-flex items-center px-4 py-2 border border-slate-200 dark:border-slate-700 text-sm font-medium rounded-xl text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 hover:bg-purple-50 hover:text-purple-600 hover:border-purple-200 dark:hover:bg-purple-900/20 dark:hover:text-purple-400 dark:hover:border-purple-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 focus:ring-offset-slate-50 dark:focus:ring-offset-slate-900 transition-all shadow-sm"
                    title="Load Project"
                  >
